...
while true; do
    ...
    if [ -n "$items" ]; then
        # Convert newlines to commas, escape existing commas if any (simple replace for now)
        comma_items=$(echo "$items" | tr '\n' ',' | sed 's/,$//')
        options="$options,$comma_items"
    fi

    # Use 'sheet' widget (available in termux-api)
    ...
    if [ "$choice" == "[OPEN SYSTEM FILE MANAGER]" ]; then
        ...
        if [ "$current_dir" != "/storage/emulated/0" ] && [ "$current_dir" != "/" ]; then
